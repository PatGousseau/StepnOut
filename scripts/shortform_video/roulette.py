    ...
    return VideoSettings(
        ...
        voice=VoiceSettings(
            ...
            spin_line=str(
                narration_raw.get(
                    "spin_line",
                    "[excited, playful] Your next side quest is being selected right now. No picking.",
                )
            ),
            reveal_lead=str(narration_raw.get("reveal_lead", "[dramatic pause] Your side quest is:")),
        ),
        output=output,
    )
...
def reveal_script(voice: VoiceSettings, winner: Candidate) -> str:
    return (
        f"{voice.reveal_lead} [direct, warm] {winner.spoken_title}. "
        f"{winner.narration}"
    )
...
def draw_roulette_frame(
    ...
    reveal_start = spin_duration + settings.roulette.reveal_delay
    ...
    card_left, card_right = 56, WIDTH - 56
    if card_y < HEIGHT - 76:
        card_bottom = min(HEIGHT - 76, card_y + 770)
        draw.rounded_rectangle((card_left + 12, card_y + 18, card_right + 12, card_bottom + 18), radius=46, fill=(77, 83, 130))
        draw.rounded_rectangle((card_left, card_y, card_right, card_bottom), radius=46, fill=(255, 255, 253), outline=(77, 83, 130), width=5)
        longest_title_line = max(winner.title.split("\n"), key=len)
        title_font = fit_font(draw, longest_title_line, 830, 106)
        draw_centered(draw, winner.title.upper(), WIDTH / 2, card_y + 76, title_font, navy, spacing=-8)

        completion_progress = phase(time, reveal_start + 1.05, reveal_start + 1.60)
        if completion_progress > 0:
            divider_y = card_y + 345
            draw.line((98, divider_y, WIDTH - 98, divider_y), fill=(229, 231, 245), width=4)
            draw_wrapped(draw, winner.completion, 98, divider_y + 52, 860, font(FONT_FUTURA, 48), navy, line_gap=14)
    add_wordmark(frame)
...
def mix_audio(
    ...
    run(
        [
            ...
            "-filter_complex",
            (
                f"[0:a]volume=1.12,adelay=0|0[intro];"
                f"[1:a]volume=1.12,adelay={delay}|{delay}[reveal];"
...
def render_visual(settings: VideoSettings, winner: Candidate, duration: float, destination: Path) -> None:
    command = [
        "ffmpeg",
        "-y",
        "-f",
    ...
    try:
        for frame_index in range(total_frames):
            ...
            draw_roulette_frame(frame, frame_index / FPS, duration, settings, winner)
...
def main() -> None:
    ...
    raw_intro_audio = work_dir / "spin_raw.mp3"
    raw_reveal_audio = work_dir / f"reveal_{winner.id}_raw.mp3"
    intro_audio = work_dir / "spin.mp3"
    ...
    if not args.skip_narration or not raw_intro_audio.exists():
        print("Generating roulette narration")
        generate_narration(settings.voice.spin_line, raw_intro_audio, settings.voice)
    if not args.skip_narration or not raw_reveal_audio.exists():
        print(f"Generating reveal narration for {winner.id}")
        generate_narration(reveal_script(settings.voice, winner), raw_reveal_audio, settings.voice)
    ...
    reveal_start = settings.roulette.spin_duration + settings.roulette.reveal_delay
    ...
    write_click_track(
        effects,
        duration,
        click_times(settings.roulette.spin_duration, total_rotation, len(settings.roulette.candidates)),
        settings.roulette.spin_duration,
    )
    mix_audio(intro_audio, reveal_audio, effects, reveal_start, duration, mixed_audio)
    render_visual(settings, winner, duration, visual)
