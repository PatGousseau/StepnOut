...
const mocks = {
  ...
  growthGuidanceService: `import {plan,step,entry} from ${JSON.stringify(fixture)};const empty=new URLSearchParams(location.search).has('empty');let activeStep=empty?null:step;let interactions=empty?[]:new URLSearchParams(location.search).has('reportonly')?[{...entry,kind:'report',report_outcome:'partly',follow_up:'harder_than_expected'}]:[entry];let latestResponse=null;
  let fail=new URLSearchParams(location.search).has('failure');
  export const growthGuidanceService={fetchCurrentPlan:async()=>null,fetchLatestInProgressIntake:async()=>null,createIntake:async()=>({id:'intake-preview'}),saveIntake:async()=>{},fetchPlanExperience:async()=>{if(fail){fail=false;throw new Error('synthetic_load_failure')}return {plan,activeStep,interactions,latestResponse,pendingInteractionId:null}},fetchJournalHistory:async()=>interactions.filter(i=>i.kind==='journal'),fetchStepHistory:async()=>[],fetchStepReports:async()=>[],
...
const result = await build({
  stdin: { contents: `import React from 'react';import {createRoot} from 'react-dom/client';import {GrowthPlanExperience} from './src/components/growthGuidance/GrowthPlanExperience';import {PersonalizedGrowthIntake} from './src/components/growthGuidance/PersonalizedGrowthIntake';import {plan} from './tests/growth-guidance-preview/fixtures';createRoot(document.getElementById('root')).render(new URLSearchParams(location.search).has('intake')?<PersonalizedGrowthIntake/>:<GrowthPlanExperience initialPlan={plan}/>);`, resolveDir: root, loader: "tsx" },
