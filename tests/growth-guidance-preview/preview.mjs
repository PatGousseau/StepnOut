...
const mocks = {
  ...
  "expo-router": 'export const router={back:()=>{location.href="/"}};',
  ...
  export const growthGuidanceService={fetchPlanExperience:async()=>{if(fail){fail=false;throw new Error('synthetic_load_failure')}return {plan,activeStep,interactions,latestResponse,pendingInteractionId:null}},fetchJournalHistory:async()=>interactions,
  setStepChoice:async(id,choice)=>{activeStep=choice==='accept'?{...step,accepted_at:new Date().toISOString()}:null},fetchVoiceJournalDraft:async()=>null,
  ...
  growthEventService: `import {eventPreferences,eventOpportunity,eventSelection} from ${JSON.stringify(fixture)};let preferences=new URLSearchParams(location.search).has('event')?eventPreferences:null;let selection=preferences?eventSelection:null;export const growthEventService={load:async()=>({preferences,areas:[],selection}),event:async()=>eventOpportunity,save:async(u,i,p)=>{preferences=p;selection=null},remove:async()=>{preferences=null;selection=null},choose:async()=>{selection=null},find:async()=>{selection=eventSelection;return selection}};`,
};
