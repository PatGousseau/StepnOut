...
try{
  ...
  const activeFixture=await must(service.from('growth_steps').insert({...step,user_id:userId,plan_id:plan.id,sequence:1}).select().single());
  ...
  const empty=await must(user.functions.invoke('find-growth-event',{body:{step_id:activeFixture.id,locale:'en'}}));
  ...
  await must(service.from('growth_event_selections').update({created_at:new Date(Date.now()-2*86400000).toISOString()}).eq('user_id',userId));
  const find=async()=>{const data=await must(user.functions.invoke('find-growth-event',{body:{step_id:activeFixture.id,locale:'en'}}));assert(data.selection.status==='proposed','Expected suitable synthetic event to be proposed');return data.selection;};
